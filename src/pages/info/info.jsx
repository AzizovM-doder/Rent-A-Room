...
const Info = () => {
  ...
  const toggleFav = () => {
    if (!item) return;
    if (liked) { removeUserFav(item.id); toast("Removed from saved", { icon: "💔" }); }
    else { addUserFav(item); toast.success("Saved to favorites!"); }
    setLiked((v) => !v);
  };
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try { await navigator.share({ title: getText(item?.name), text: `Check out this incredible listing on Rent.A.Room`, url }); }
      catch {}
    } else {
        ...
        <Card className="max-w-md w-full rounded-[2.5rem] text-center overflow-hidden border-0 shadow-2xl bg-background/80 backdrop-blur-xl">
          ...
          <CardContent className="p-10 flex flex-col items-center gap-5 -mt-16 relative z-10">
            ...
            <div>
              ...
              <p className="text-muted-foreground font-medium">This property does not exist, has been removed by the host, or is currently private.</p>
            </div>
            <Button asChild className="mt-4 h-14 w-full rounded-2xl bg-emerald-600 hover:bg-emerald-700 shadow-lg shadow-emerald-500/20 text-base font-bold">
              <Link to="/"><ArrowLeft className="h-5 w-5 mr-2" />Return to Discover</Link>
            </Button>
          </CardContent>
        </Card>
      </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-32">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between mb-8">
        <Button variant="ghost" asChild className="hover:bg-muted text-muted-foreground hover:text-foreground group h-10 px-3 -ml-3">
          <Link to="/"><ArrowLeft className="h-4 w-4 mr-2 group-hover:-translate-x-1 transition-transform" />Back to listings</Link>
        </Button>
      ...
      <motion.div variants={fadeUp} initial="hidden" animate="visible" className="mb-8">
        ...
        <div className="flex flex-wrap items-center gap-y-2 gap-x-4 text-sm font-medium text-muted-foreground">
          <span className="flex items-center gap-1.5 text-foreground/90">
             <Star className="h-4 w-4 fill-amber-400 text-amber-400" /> 4.98 <span className="text-muted-foreground underline ml-1 cursor-pointer">(124 reviews)</span>
          </span>
          <span className="hidden sm:inline text-border">•</span>
          <span className="flex items-center gap-1.5">
             <ShieldCheck className="h-4 w-4 text-emerald-500" /> Superhost
          </span>
          <span className="hidden sm:inline text-border">•</span>
      ...
      >
        ...
        <Button variant="secondary" className="absolute bottom-6 right-6 rounded-full shadow-xl bg-white/90 hover:bg-white text-black font-bold border-0 backdrop-blur-md opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-300">
           <Map className="h-4 w-4 mr-2" /> Show Map
        </Button>
      </motion.div>
      ...
      <div className="grid lg:grid-cols-12 gap-12 lg:gap-16 relative">
        ...
        <div className="lg:col-span-8 flex flex-col gap-10">
          ...
          <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} className="flex items-center justify-between pb-8 border-b border-border/50">
            <div>
              <h2 className="text-2xl font-extrabold mb-1">Entire {getText(item.type) || "home"} hosted by Rent.A.Room</h2>
              <p className="text-muted-foreground flex items-center gap-2 text-sm font-medium">
                {item.rooms} beds <span className="text-border">•</span> 1 bath <span className="text-border">•</span> Fast Wifi
              </p>
            </div>
          ...
          <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} className="flex flex-col gap-6 py-4">
             <div className="flex gap-4">
                ...
                <div>
                   <h3 className="font-bold text-lg leading-none mb-1">Entire place</h3>
                   <p className="text-muted-foreground text-sm">You'll have the space to yourself.</p>
                </div>
             </div>
             <div className="flex gap-4">
                <div className="mt-1"><CheckCircle2 className="h-7 w-7 text-foreground/80" /></div>
                <div>
                   <h3 className="font-bold text-lg leading-none mb-1">Enhanced Clean</h3>
                   <p className="text-muted-foreground text-sm">This Host committed to a 5-step enhanced cleaning process.</p>
                </div>
             </div>
             <div className="flex gap-4">
                <div className="mt-1"><Calendar className="h-7 w-7 text-foreground/80" /></div>
                <div>
                   <h3 className="font-bold text-lg leading-none mb-1">Free cancellation for 48 hours</h3>
                   <p className="text-muted-foreground text-sm">Lock in this price and cancel without penalty.</p>
                </div>
             </div>
          </motion.div>
          ...
          <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }}>
            <h2 className="text-2xl font-extrabold mb-4">About this space</h2>
            <div className="prose prose-emerald dark:prose-invert max-w-none">
              {item.about ? (
                <p className="text-muted-foreground whitespace-pre-wrap leading-relaxed text-[15px] font-medium">{item.about}</p>
              ) : (
                <p className="text-muted-foreground italic bg-muted/50 p-6 rounded-2xl border border-dashed border-border">The host hasn't provided a detailed description yet.</p>
              )}
            </div>
          </motion.div>
               ...
               <CardContent className="p-6 sm:p-8 flex flex-col gap-6 relative z-10 bg-background rounded-[calc(2rem-2px)] h-full">
                 ...
                 <div className="flex items-end justify-between">
                    <div>
                      ...
                      <span className="text-base font-medium text-muted-foreground ml-1">night</span>
                    </div>
                 ...
                 <div className="rounded-2xl border border-border/60 overflow-hidden text-sm">
                   <div className="flex divide-x border-b border-border/60">
                     <div className="flex-1 p-3 hover:bg-muted/30 cursor-pointer transition-colors">
                       <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-600 mb-0.5">Check-in</p>
                       <p className="font-medium">Add date</p>
                     </div>
                     <div className="flex-1 p-3 hover:bg-muted/30 cursor-pointer transition-colors">
                       <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-600 mb-0.5">Checkout</p>
                       <p className="font-medium">Add date</p>
                     </div>
                   </div>
                   <div className="p-3 hover:bg-muted/30 cursor-pointer transition-colors flex justify-between items-center">
                      <div>
                        <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-600 mb-0.5">Guests</p>
                 ...
                 <div className="flex flex-col gap-3">
                   <Button asChild className="w-full h-14 rounded-xl bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-700 hover:to-emerald-600 text-white font-bold text-lg shadow-[0_8px_30px_rgb(16,185,129,0.3)] hover:shadow-[0_8px_30px_rgb(16,185,129,0.5)] transition-all hover:-translate-y-0.5 border-0">
                     <Link to={`/massage/${id}`}>Reserve Space</Link>
                   </Button>
                   <p className="text-center text-xs text-muted-foreground font-medium mt-1">You won't be charged yet</p>
                 </div>
                 ...
                 <div className="flex flex-col gap-3 text-sm font-medium text-muted-foreground">
                    <div className="flex justify-between items-center">
                      <span className="underline"> ${item.price} x 5 nights</span>
